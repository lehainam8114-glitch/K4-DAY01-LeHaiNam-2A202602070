...
**Phân loại ảnh.** Ground truth là một nhãn lớp duy nhất cho toàn ảnh, gồm `class_id`, `class_name` và `taxonomy_name`. Điểm mơ hồ quan sát được: ảnh `traffic` chứa nhiều loại xe cùng lúc — model dự đoán `cab` (score 0.51) nhưng xe buýt xanh chiếm diện tích lớn hơn; không rõ chọn lớp nào là đúng nếu không có guideline. Annotator đọc guideline để xác định tiêu chí chọn chủ thể chính, chọn đúng một lớp, ghi chú "ambiguous" và escalate nếu không chắc. Reviewer kiểm tra nhãn có khớp với chủ thể nổi bật nhất theo guideline không; nếu có ghi chú "ambiguous" thì quyết định hoặc escalate lên lead.

**Phát hiện vật thể.** Ground truth là danh sách hộp `bbox_xyxy` kèm `class_id` và `class_name`, mỗi hộp cho một vật thể. Điểm mơ hồ quan sát được: `kitchen` có person bên trái bị cắt mép ảnh (`x_min ≈ 0`), nhiều bowl nhỏ chồng lên nhau; một số vật thể có score thấp bị ngưỡng 0.35 lọc ra nhưng guideline vẫn yêu cầu gán nhãn. Annotator vẽ tight bbox cho tất cả vật thể đủ điều kiện kể cả khi model bỏ sót, ghi chú trường hợp bị cắt mép hoặc che khuất, escalate nếu không rõ. Reviewer kiểm tra hộp có bám sát vật thể không (không thừa nền, không thiếu phần vật thể), kiểm tra nhãn lớp đúng chưa, xác nhận xử lý vật thể cắt mép và che khuất đúng quy tắc.

**Instance segmentation.** Ground truth là danh sách polygon `[x, y]` khép kín kèm `instance_id`, `class_id` và `class_name`, mỗi polygon cho một đối tượng riêng lẻ. Điểm mơ hồ quan sát được: `kitchen-005` (dining_table, 598 điểm) có biên bàn tiếp xúc với bát và nền — không rõ polygon dừng ở đâu; `kitchen-002` (bowl) bị che khuất — không rõ chỉ vẽ phần nhìn thấy hay ước lượng phần khuất. Annotator vẽ polygon bám sát biên nhìn thấy (visible contour only), dùng instance_id để phân biệt các vật thể cùng lớp, không ước đoán phần khuất, escalate khi biên mơ hồ. Reviewer kiểm tra polygon bám sát biên thực không quá 3 pixel, kiểm tra instance_id không bị trùng, xác nhận vùng tiếp xúc và che khuất xử lý đúng quy tắc, loại mask nếu biên lệch quá giới hạn cho phép.
